...
protocol ZigzagLayoutDelegate {
    func collectionView(_ collectionView: UICollectionView, heightForImageAtIndexPath indexPath: IndexPath) -> CGFloat
}

class ZigzagLayout: UICollectionViewLayout {
    
    var delegate: ZigzagLayoutDelegate!
    private var numberOfColumns = 2
    private var cellPadding: CGFloat = 5.5
    ...
    override func prepare() {
        ...
        for item in 0 ..< collectionView.numberOfItems(inSection: 0) {
            ...
            let height = linePadding * 2 + photoHeight
            let frame = CGRect(x: xOffset[column], y: yOffset[column], width: columnWidth, height: height)
            let insetFrame = frame.insetBy(dx: cellPadding, dy: linePadding)
